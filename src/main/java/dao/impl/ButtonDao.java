...
public class ButtonDao {
    ...
    public int getCommandId(String text) {
        try {
            String selectButtonByText = "SELECT command_id FROM PUBLIC.BUTTON WHERE TEXT=?";
            PreparedStatement ps = connection.prepareStatement(selectButtonByText);
            ps.setString(1, text);
            ps.execute();
            ResultSet rs = ps.getResultSet();
            rs.next();
